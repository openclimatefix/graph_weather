...
def benchmark_full_model(
    ...
    try:
        ...
        encoder = Encoder(lat_lons, resolution=2, input_dim=78, output_dim=256, efficient_batching=True).eval()
        processor = Processor(256, hidden_dim_processor_edge=256, hidden_dim_processor_node=256, hidden_layers_processor_edge=2, hidden_layers_processor_node=2, num_blocks=9).eval()
        decoder = Decoder(lat_lons, resolution=2, input_dim=256, output_dim=78, efficient_batching=True).eval()
        ...
        with torch.no_grad():
            ...
            x_proc = processor(x, edge_idx_enc, edge_attr_enc, batch_size=batch_size, efficient_batching=True)
        ...
        results = {
            ...
            'input_cpu_memory_mb': mem_after_input['cpu_memory_mb'] - mem_after_model['cpu_memory_mb'],
            'forward_cpu_memory_mb': mem_after_forward['cpu_memory_mb'] - mem_after_input['cpu_memory_mb'],
            'total_cpu_memory_mb': mem_after_forward['cpu_memory_mb'] - mem_before['cpu_memory_mb'],

            'model_gpu_memory_mb': mem_after_model['gpu_memory_allocated_mb'] - mem_before['gpu_memory_allocated_mb'],
            'input_gpu_memory_mb': mem_after_input['gpu_memory_allocated_mb'] - mem_after_model['gpu_memory_allocated_mb'],
            'forward_gpu_memory_mb': mem_after_forward['gpu_memory_allocated_mb'] - mem_after_input['gpu_memory_allocated_mb'],
            'total_gpu_memory_mb': mem_after_forward['gpu_memory_allocated_mb'] - mem_before['gpu_memory_allocated_mb'],
            ...
            'total_avg_time_s': np.mean(encoder_times) + np.mean(processor_times) + np.mean(decoder_times),
            ...
            'edge_index_memory_original_mb': (encoder.graph.edge_index.element_size() *
                                    encoder.graph.edge_index.nelement()) / 1024 / 1024,
            'edge_attr_memory_original_mb': (encoder.graph.edge_attr.element_size() *
                                   encoder.graph.edge_attr.nelement()) / 1024 / 1024,
            'edge_index_memory_batched_mb': np.mean(batched_edge_index_sizes),
            'edge_attr_memory_batched_mb': np.mean(batched_edge_attr_sizes),
            'batching_overhead_ratio': np.mean(batched_edge_index_sizes) / ((encoder.graph.edge_index.element_size() *
                                    encoder.graph.edge_index.nelement()) / 1024 / 1024) if encoder.graph.edge_index.nelement() > 0 else 0,
        }
...
def run_benchmark_suite():
    ...
    for resolution_deg, batch_sizes in test_configs:
        ...
        for batch_size in batch_sizes:
            ...
            if result['success']:
                ...
                print(f"    Edge index (original): {result['edge_index_memory_original_mb']:.2f} MB")
                ...
                print(f"    Encoder:    {result['encoder_avg_time_s']*1000:.2f} ± {result['encoder_std_time_s']*1000:.2f} ms")
                print(f"    Processor:  {result['processor_avg_time_s']*1000:.2f} ± {result['processor_std_time_s']*1000:.2f} ms")
                print(f"    Decoder:    {result['decoder_avg_time_s']*1000:.2f} ± {result['decoder_std_time_s']*1000:.2f} ms")
            ...
            if not result['success'] and 'out of memory' in result.get('error', '').lower():
                print(f"  Skipping larger batch sizes due to OOM\n")
                break
    ...
    print(f"{'Res':<6} {'Batch':<6} {'Nodes':<8} {'Edges':<8} {'Batch OH':<10} {'Enc(ms)':<10} {'Proc(ms)':<10} {'Dec(ms)':<10} {'Peak MB':<10} {'Status':<8}")
    ...
    for result in all_results:
        ...
        batch_oh = f"{result.get('batching_overhead_ratio', 0):.2f}x" if result['success'] else "N/A"
        enc_time = f"{result.get('encoder_avg_time_s', 0)*1000:.1f}" if result['success'] else "N/A"
        proc_time = f"{result.get('processor_avg_time_s', 0)*1000:.1f}" if result['success'] else "N/A"
        dec_time = f"{result.get('decoder_avg_time_s', 0)*1000:.1f}" if result['success'] else "N/A"
        peak_mem = f"{result.get('peak_gpu_memory_mb', 0):.1f}" if result['success'] and device == 'cuda' else "N/A"
        status = "OK" if result['success'] else "FAIL"

        print(f"{res:<6} {batch:<6} {nodes:<8} {edges:<8} {batch_oh:<10} {enc_time:<10} {proc_time:<10} {dec_time:<10} {peak_mem:<10} {status:<8}")
    ...
    if one_deg_results:
        ...
        for r in one_deg_results:
            print(f"  Batch {r['batch_size']}: {r['batching_overhead_ratio']:.2f}x memory overhead from batching")
    else:
        print(f"✗ 1-degree grid: No successful runs (even batch_size=1 failed!)")
    ...
    if successful and device == 'cuda':
        ...
        for r in successful:
            if r['edge_index_memory_original_mb'] > 0:
                ...
                print(f"  {r['resolution_deg']:.1f}° batch={r['batch_size']}: +{overhead_mb:.2f} MB ({r['batching_overhead_ratio']:.2f}x original)")
