...
def main():
    ...
    plot_training_curves(
        ...
        results['val_losses'], 
        title="Data Assimilation Training Curves"
    )
    ...
    for key, value in eval_metrics.items():
        if 'avg_' in key and ('rmse' in key or 'mae' in key or 'bias' in key or 'correlation' in key):
    ...
    print("SUMMARY")
    print("="*50)
    print(f"✓ Successfully trained a self-supervised data assimilation model")
    print(f"✓ Model learned to minimize 3D-Var cost function without ground truth")
    print(f"✓ Analysis RMSE: {comparison['avg_analysis_rmse']:.4f}")
    print(f"✓ Background RMSE: {comparison['avg_background_rmse']:.4f}")
    print(f"✓ Analysis improvement over background: {comparison['analysis_improvement_over_bg']:.2f}%")
    print(f"✓ Analysis improvement over observations: {comparison['analysis_improvement_over_obs']:.2f}%")
    
    print(f"\nThe model successfully learned to combine background and observations")
    print(f"optimally to produce better analysis states than either input alone!")
