...
def test_basic_3dvar_loss():
    ...
    loss_fn_custom = ThreeDVarLoss(background_error_covariance=B, observation_error_covariance=R)
...
def test_training_pipeline():
    ...
    model = SimpleDataAssimilationModel(
        grid_size=(8, 8), num_channels=1, hidden_dim=32, num_layers=2
    )
    ...
    plot_training_curves(
        results["train_losses"], results["val_losses"], title="Test Training Curves"
    )
...
def test_comparison_with_baselines(trainer, data_module):
    ...
    comparison = compare_with_baselines(trainer.model, data_module.test_dataloader(), device="cpu")
...
def test_visualization_pipeline():
    ...
    background, observations, true_state = generate_synthetic_data(
        batch_size=batch_size, grid_size=grid_size, num_channels=1
    )
...
def run_comprehensive_test():
    ...
    print(
        f"- Analysis improvement over background: {comparison_results['analysis_improvement_over_bg']:.2f}%"
    )
    print(
        f"- Analysis improvement over observations: {comparison_results['analysis_improvement_over_obs']:.2f}%"
    )
...
def test_different_training_modes():
    ...
    results = train_with_different_modes(
        model_class=lambda **kwargs: SimpleDataAssimilationModel(
            grid_size=(6, 6), num_channels=1, hidden_dim=16, num_layers=2
        ),
        data_module=AssimilationDataModule,
        grid_size=(6, 6),
        num_channels=1,
