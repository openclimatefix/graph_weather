...
class ThreeDVarLoss(nn.Module):
    ...
    def forward(self, analysis, background, observations):
        ...
        if self.B_inv is None:
            ...
            bg_term = torch.sum(bg_diff * torch.matmul(bg_diff.unsqueeze(-2), self.B_inv).squeeze(-2), dim=-1)
        ...
        if self.R_inv is None:
            ...
            obs_term = torch.sum(obs_diff * torch.matmul(obs_diff.unsqueeze(-2), self.R_inv).squeeze(-2), dim=-1)
