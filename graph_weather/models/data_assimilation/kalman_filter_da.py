...
class KalmanFilterDA(DataAssimilationBase):
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        self.ensemble_generator = EnsembleGenerator(
            noise_std=self.background_error_std,
            method='gaussian'
        )
    ...
    def _assimilate_tensor_ensemble(
        ...
        ).unsqueeze(0).expand(batch_size, -1, -1)  # [batch_size, obs_dim, obs_dim]
        ...
        ) / (num_members - 1)  # [batch_size, obs_dim, obs_dim]
        ...
        updated_mean = inflated_ensemble.mean(dim=1) + dx.unsqueeze(1)  # [batch_size, state_dim]
        updated_ensemble = inflated_ensemble + (
            updated_mean.unsqueeze(1) - inflated_ensemble.mean(dim=1, keepdim=True)
        )
    ...
    def _compute_analysis(
        ...
        if isinstance(ensemble, torch.Tensor):
            ...
            for edge_type in ensemble.edge_types:
                for key, value in ensemble[edge_type].items():
                    if (key != 'edge_attr' or 
                        ensemble[edge_type].edge_attr is None or 
                        ensemble[edge_type].edge_attr.dim() != 3):
                        setattr(result[edge_type], key, value)
                    else:
