...
class IntegrationLayer:
    ...
    def _initialize_fengwu_model(self) -> Union[ImageMetaModel, WrapperImageModel]:
        ...
        if self.fengwu_ghr_config.scale_factor:
            return WrapperImageModel(base_model, scale_factor=self.fengwu_ghr_config.scale_factor)
    ...
    def _init_transform_patterns(self):
        ...
        self.transform_patterns = {
            ...
            "b_n_c_to_b_c_h_w": lambda x: rearrange(
                x, "b (h w) c -> b c h w", h=self.num_lat, w=self.num_lon
            ),
            "b_h_w_c_to_b_n_c": lambda x: rearrange(x, "b h w c -> b (h w) c"),
            "b_n_c_to_b_h_w_c": lambda x: rearrange(
                x, "b (h w) c -> b h w c", h=self.num_lat, w=self.num_lon
            ),
        }
    ...
    def _validate_grid(self, grid: np.ndarray, grid_type: str) -> np.ndarray:
        ...
        if grid_type == "longitude":
            if not (grid >= 0).all() or not (grid < 360).all():
                raise ValidationError("Longitude values must be between 0 and 360")
    ...
    def generate_noise(
        self, num_samples: int = 1, isotropic: bool = True, seed: Optional[int] = None
    ) -> np.ndarray:
        """
    ...
    def preprocess_coordinates(
        self, lat_lons: list, validate: bool = True
    ...
    def transform_tensor(
        self, x: torch.Tensor, source_format: str, target_format: str
    ) -> torch.Tensor:
        """
    ...
    def interpolate_features(
        ...
        try:
            ...
            den = scatter(
                weights, y_idx, 0, pos_y.size(0), reduce="sum"
            )  # Denominator: sum of weights

            # Ensure that x[x_idx] * weights is correctly dimensioned to match pos_y size
            weighted_features = scatter(
                x[x_idx] * weights, y_idx, 0, pos_y.size(0), reduce="sum"
            )  # Weighted features
