...
def compare_methods(model_analysis, background, observations, true_state):
    ...
    results['analysis_improvement_over_bg_pct'] = (
        (bg_rmse - analysis_rmse) / bg_rmse * 100
    ) if bg_rmse > 0 else 0
    
    results['analysis_improvement_over_obs_pct'] = (
        (obs_rmse - analysis_rmse) / obs_rmse * 100
    ) if obs_rmse > 0 else 0
    
    results['bg_improvement_over_obs_pct'] = (
        (obs_rmse - bg_rmse) / obs_rmse * 100
    ) if obs_rmse > 0 else 0
...
def compute_cross_validation_score(model, data_loader, k_folds=5):
    ...
    with torch.no_grad():
        for batch in data_loader:
            ...
            observations = batch['observations'].to(model.device if hasattr(model, 'device') else 'cpu')
            
            if 'true_state' in batch:
                true_state = batch['true_state'].to(model.device if hasattr(model, 'device') else 'cpu')
