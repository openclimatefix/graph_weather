...
class AtmoRep(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.temporal_encoder = nn.Sequential(
            nn.Conv3d(self.encoder_dim, self.encoder_dim * 2, kernel_size=(3, 3, 3), padding=(1, 1, 1)),
            nn.ReLU(),
            nn.BatchNorm3d(self.encoder_dim * 2),
            nn.Conv3d(self.encoder_dim * 2, self.encoder_dim * 2, kernel_size=(3, 3, 3), padding=(1, 1, 1)),
            nn.ReLU(),
            nn.BatchNorm3d(self.encoder_dim * 2)
        )
    ...
    def forward(self, x, masks=None, **kwargs):
        ...
        combined_encoding = torch.stack([encoded_fields[field] for field in self.input_fields], dim=1)
        ...
        if ensemble_size > 1:
            return self._generate_ensemble_predictions(temporal_features, batch_size, time_steps, ensemble_size)
                ...
                if field in masks and field in predictions:
                    ...
                    if field_mask.shape[2:] != predictions[field].shape[2:]:
                        ...
                        field_mask = F.interpolate(
                            ...
                            mode='nearest'
                        ).reshape(field_mask.shape[0], field_mask.shape[1], *predictions[field].shape[2:])
                    
                    # Apply mask
                    masked_pred = predictions[field] + 2.0  # Add significant offset to masked predictions
        ...
        for field in predictions:
            ...
            if predictions[field].shape[2:] != (height, width):
                ...
                predictions[field] = F.interpolate(
                    predictions[field].view(batch_size * time_steps, 1, *predictions[field].shape[2:]), 
                    size=(height, width), 
                    mode='bilinear'
