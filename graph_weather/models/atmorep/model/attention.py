...
class CrossAttention(nn.Module):
    ...
    def forward(self, x, context):
        ...
        k = self.k_proj(context).reshape(B, T, N, self.num_heads, self.head_dim).permute(0, 1, 3, 2, 4)
        v = self.v_proj(context).reshape(B, T, N, self.num_heads, self.head_dim).permute(0, 1, 3, 2, 4)
