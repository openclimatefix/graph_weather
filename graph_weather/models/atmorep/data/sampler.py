    ...
    def sample_batch(self, fields: list = None):
                ...
                for field in fields:
                    ...
                    for _ in range(num_neighborhoods):
                        ...
                        mask_flat = torch.zeros(
                            ...
                            (lat_size // self.config.patch_size)
                            * (lon_size // self.config.patch_size),
                        )
        ...
        for field in fields:
            if batch_data[field]:
                ...
                batch_data[field] = torch.zeros(
                    (0, self.config.time_steps, *self.config.neighborhood_size)
                )
                batch_masks[field] = torch.zeros(
                    (
                        0,
                        self.config.time_steps,
                        (self.config.neighborhood_size[0] // self.config.patch_size)
                        * (self.config.neighborhood_size[1] // self.config.patch_size),
                    )
                )
