...
class GenDA(torch.nn.Module, PyTorchModelHubMixin):
    """GenDA diffusion model with sensor conditioning."""
    ...
    def __init__(
        ...
        Args:
            ...
            conditioning_dim (int): dimension of the conditioning features used for
                sensor guidance.
        """
        super().__init__()
        ...
        self.encoder = Encoder(
            grid_dim=(
                output_features_dim
                + 2 * input_features_dim
                + conditioning_dim
                + self.graphs.grid_nodes_dim
            ),
    ...
    def forward(
        ...
        Args:
            ...
            sensor_mask (torch.Tensor, optional): mask indicating available
                sensor observations.

            sensor_values (torch.Tensor, optional): sensor observation values
                used for conditioning.

            wind_direction (torch.Tensor, optional): optional wind direction
                conditioning input.
        """
        ...
        if sensor_mask is not None:
            sensor_mask = einops.rearrange(
                sensor_mask,
                "b lon lat f -> b (lon lat) f",
            )

        if sensor_values is not None:
            sensor_values = einops.rearrange(
                sensor_values,
                "b lon lat f -> b (lon lat) f",
            )
        ...
        if (
            ...
            and sensor_values is not None
            and torch.rand(1).item() < 0.1
        ):
            sensor_mask = torch.zeros_like(sensor_mask)
            sensor_values = torch.zeros_like(sensor_values)
        # concatenate inputs and targets across features dimension.
        conditioning = []

        if sensor_mask is not None:
            conditioning.append(sensor_mask)

        if sensor_values is not None:
            conditioning.append(sensor_values)

        if len(conditioning) > 0:
            conditioning = torch.cat(conditioning, dim=-1)

            grid_features = torch.cat(
                (scaled_targets, prev_inputs, conditioning),
                dim=-1,
            )
        else:
            grid_features = torch.cat(
                (scaled_targets, prev_inputs),
                dim=-1,
            )
    ...
    def _register_graph(self):
        """Register graph tensors as non-persistent buffers."""
    ...
    def guided_forward(
        ...
        """Run conditional diffusion guidance."""
        cond = self.forward(
            corrupted_targets,
            prev_inputs,
            noise_levels,
