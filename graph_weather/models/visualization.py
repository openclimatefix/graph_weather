...
def plot_comparison_grid(background, observations, analysis, true_state=None, 
...
def plot_error_maps(background, observations, analysis, true_state, 
                   titles=None, figsize=(18, 5)):
    """
    ...
    max_error = max(np.abs(bg_error).max(), 
                    np.abs(obs_error).max(), 
                    np.abs(analysis_error).max())
    
    # Plot error maps
    im1 = axes[0].imshow(bg_error if bg_error.ndim == 2 else bg_error[0], 
                        cmap='RdBu_r', vmin=-max_error, vmax=max_error)
    axes[0].set_title(titles[0])
    axes[0].axis('off')
    plt.colorbar(im1, ax=axes[0])
    
    im2 = axes[1].imshow(obs_error if obs_error.ndim == 2 else obs_error[0], 
                        cmap='RdBu_r', vmin=-max_error, vmax=max_error)
    axes[1].set_title(titles[1])
    axes[1].axis('off')
    plt.colorbar(im2, ax=axes[1])
    
    im3 = axes[2].imshow(analysis_error if analysis_error.ndim == 2 else analysis_error[0], 
                        cmap='RdBu_r', vmin=-max_error, vmax=max_error)
    axes[2].set_title(titles[2])
    axes[2].axis('off')
    plt.colorbar(im3, ax=axes[2])
...
def plot_histogram_comparison(true_state, background, analysis, bins=50, 
...
def plot_scatter_comparison(true_state, background, analysis, 
...
def plot_parameter_analysis(model, title="Parameter Analysis"):
    ...
    plt.ylabel('L2 Norm')
    plt.title(title)
    plt.xticks(range(len(param_names)), [name.split('.')[-1] for name in param_names], 
               rotation=45, ha='right')
    
    # Add value labels on bars
    for bar, value in zip(bars, param_norms):
        plt.text(bar.get_x() + bar.get_width()/2, bar.get_height() + max(param_norms)*0.01,
...
def create_summary_dashboard(metrics, figsize=(16, 12)):
    ...
    if improvement_metrics:
        bars = ax5.bar(improvement_metrics, improvement_values, 
        ...
        for bar, value in zip(bars, improvement_values):
            ...
            ax5.text(bar.get_x() + bar.get_width()/2, height + (max(improvement_values)*0.01 if max(improvement_values) > 0 else min(improvement_values)*0.01),
    ...
    if 'avg_information_gain' in metrics:
        ...
        ax6.set_ylabel('Gain (%)')
        ax6.text(0, info_gain + max(info_gain*0.01, 0.1), f'{info_gain:.1f}%', 
                ha='center', va='bottom')
    ...
    if 'model' in metrics:
        ...
        if param_norms:  # Only plot if there are parameters to show
            ...
            ax7.text(0.5, 0.5, 'No parameters to display', horizontalalignment='center', 
                    verticalalignment='center', transform=ax7.transAxes, fontsize=14)
...
def visualize_observation_locations(observations, obs_mask, title="Observation Locations"):
    ...
    obs_visual = torch.zeros_like(observations[0, 0]) if len(observations.shape) > 2 else torch.zeros_like(observations[0])
