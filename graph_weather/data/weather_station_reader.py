...
class WeatherStationReader:
    ...
    def _convert_to_synopticpy(self, observations: xr.Dataset) -> Dict:
        ...
        for station in observations.station.values:
            ...
            elevation = float(station_data.elevation.values) if 'elevation' in station_data else None
            ...
            for var_name in observations.data_vars:
                if var_name not in ['lat', 'lon', 'elevation'] and 'time' in observations[var_name].dims:
                    # Extract values for this station and variable
                    values = station_data[var_name].values
                    
                    # Add to observations
                    synoptic_data["STATION"][str(station)]["OBSERVATIONS"][var_name] = values.tolist()
        
        return synoptic_data
    def convert_to_model_format(
        self, 
        observations: Optional[xr.Dataset], 
        model_format: str = "weatherreal"
    ...
    def _convert_to_weatherreal(self, observations: xr.Dataset) -> xr.Dataset:
        ...
        for var_name, unit in default_units.items():
            if var_name in weatherreal_data.data_vars and "units" not in weatherreal_data[var_name].attrs:
    ...
    def initialize_synopticpy(self, token: Optional[str] = None, token_path: Optional[str] = None) -> bool:
        """
        ...
        try:
            ...
            if isinstance(data, dict) and 'STATION' in data:
                ...
                for station_id, station_info in data['STATION'].items():
                    ...
                    if 'OBSERVATIONS' in station_info and 'date_time' in station_info['OBSERVATIONS']:
    ...
    def create_empty_weatherreal_dataset(
        ...
        for var_name in variables:
            data_vars[var_name] = (['time', 'station'], 
