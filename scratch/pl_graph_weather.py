...
def process_data(data):
    data.update(
        {
            ...
            if key.replace("current_", "").replace("next_", "") in means.keys() and np.asarray(value).ndim == 2
        }
    ...
    output_data = {
        key.replace("next_", ""): torch.from_numpy((value - means[key.replace("next_", "")]) / stds[key.replace("next_", "")])
        for key, value in data.items()
    ...
    solar_times = [
        np.array([extraterrestrial_irrad(when=date.to_pydatetime(), latitude_deg=lat, longitude_deg=lon) for lat, lon in lat_lons])
    ]
    for when in pd.date_range(date - pd.Timedelta("12 hours"), date + pd.Timedelta("12 hours"), freq=f"1H"):
        solar_times.append(
            np.array(
                [extraterrestrial_irrad(when=when.to_pydatetime(), latitude_deg=lat, longitude_deg=lon) for lat, lon in lat_lons]
            )
        )
    ...
    new_data = {
        ...
        "has_nans": not np.isnan(input_data.float().numpy()).any() and not np.isnan(output_data.float().numpy()).any(),
    }
    return new_data
...
class GraphDataModule(pl.LightningDataModule):
    def __init__(self, deg: str = "2.0", batch_size: int = 1):
        ...
        self.dataset = datasets.load_dataset("openclimatefix/gfs-surface-pressure-2deg", split="train+validation", streaming=False)
        ...
        self.dataset = (
            self.dataset.map(
                process_data, remove_columns=self.dataset.column_names, features=features, num_proc=16, writer_batch_size=2
            )
...
class LitGraphForecaster(pl.LightningModule):
    def __init__(
        self, lat_lons: list, feature_dim: int = 605, aux_dim: int = 32, hidden_dim: int = 64, num_blocks: int = 3, lr: float = 3e-4
    ):
        super().__init__()
...
def run(num_blocks, hidden, batch, gpus):
    ...
    lat_lons = np.array(
        np.meshgrid(np.asarray(example_batch["latitude"]).flatten(), np.asarray(example_batch["longitude"]).flatten())
    ).T.reshape((-1, 2))
