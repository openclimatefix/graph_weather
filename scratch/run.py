...
class XrDataset(IterableDataset):
    def __init__(self, resolution="2.0deg"):
        ...
        features = Features(
            {
                ...
                "current_10m_above_ground": Array3D(shape=(LATITUDE, LONGITUDE, 2), dtype="float32"),
                "next_20m_above_ground": Array3D(shape=(LATITUDE, LONGITUDE, 2), dtype="float32"),
                "current_20m_above_ground": Array3D(shape=(LATITUDE, LONGITUDE, 2), dtype="float32"),
                "next_30m_above_ground": Array3D(shape=(LATITUDE, LONGITUDE, 2), dtype="float32"),
                "current_30m_above_ground": Array3D(shape=(LATITUDE, LONGITUDE, 2), dtype="float32"),
                "next_40m_above_ground": Array3D(shape=(LATITUDE, LONGITUDE, 2), dtype="float32"),
                "current_40m_above_ground": Array3D(shape=(LATITUDE, LONGITUDE, 2), dtype="float32"),
                "next_50m_above_ground": Array3D(shape=(LATITUDE, LONGITUDE, 2), dtype="float32"),
                "current_50m_above_ground": Array3D(shape=(LATITUDE, LONGITUDE, 2), dtype="float32"),
                "next_80m_above_ground": Array3D(shape=(LATITUDE, LONGITUDE, 5), dtype="float32"),
                "current_80m_above_ground": Array3D(shape=(LATITUDE, LONGITUDE, 5), dtype="float32"),
                "next_100m_above_ground": Array3D(shape=(LATITUDE, LONGITUDE, 3), dtype="float32"),
                "current_100m_above_ground": Array3D(shape=(LATITUDE, LONGITUDE, 3), dtype="float32"),
        ...
        self.dataset = datasets.load_dataset(
            "openclimatefix/gfs-surface-pressure-2.0deg", split="train", streaming=True, features=features
        )
        ...
        for data in iter(self.dataset):
            ...
            data.update(
                {
                    ...
                    if key.replace("current_", "").replace("next_", "") in self.means.keys() and np.asarray(value).ndim == 2
                }
            )  # Add third dimension for ones with 2
            input_data = {
                key.replace("current_", ""): torch.from_numpy(
                    (value - self.means[key.replace("current_", "")]) / self.stds[key.replace("current_", "")]
                )
                for key, value in data.items()
            ...
            output_data = {
                key.replace("next_", ""): torch.from_numpy(
                    (value - self.means[key.replace("next_", "")]) / self.stds[key.replace("next_", "")]
                )
                for key, value in data.items()
            ...
            if self.landsea_fixed is None:
                ...
                landsea = self.landsea.interp(latitude=np.asarray(data["latitude"]).flatten()).interp(
                    longitude=np.asarray(data["longitude"]).flatten()
                )
            ...
            solar_times = [
                np.array(
                    [
                        extraterrestrial_irrad(when=date.to_pydatetime(), latitude_deg=lat, longitude_deg=lon)
            ...
            for when in pd.date_range(date - pd.Timedelta("12 hours"), date + pd.Timedelta("12 hours"), freq=f"1H"):
                solar_times.append(
                    np.array(
                        [
                            extraterrestrial_irrad(when=when.to_pydatetime(), latitude_deg=lat, longitude_deg=lon)
                            for lat, lon in lat_lons
                        ]
                    )
                )
...
for epoch in range(100):  # loop over the dataset multiple times
    ...
    for i, data in enumerate(dataset):
        ...
        if i % 10 == 0:
            ...
            model.push_to_hub(
                "graph-weather-forecaster-2.0deg", organization="openclimatefix", commit_message=f"Add model Epoch={epoch}, i={i}"
            )
    if epoch % 5 == 0:
        assert not np.isnan(running_loss)
        model.push_to_hub(
