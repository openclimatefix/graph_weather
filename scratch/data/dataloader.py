...
class AnalysisDataset(Dataset):
    ...
    def __getitem__(self, item):
        ...
        end_solar_times = [np.array([extraterrestrial_irrad(end_date, lat, lon) for lat, lon in lat_lons])]
        ...
        input_data = np.stack(
            [start[f"{var}"].values for var in start.data_vars if not np.isnan(start[f"{var}"].values).any()],
            axis=-1,
        )
        ...
        output_data = np.stack(
            [end[f"{var}"].values for var in end.data_vars if not np.isnan(end[f"{var}"].values).any()],
            axis=-1,
        )
...
obs_data = xr.open_zarr("/home/jacob/Development/prepbufr.gdas.20160101.t00z.nr.48h.raw.zarr", consolidated=True)
...
exit()
analysis_data = xr.open_zarr("/home/jacob/Development/gdas1.fnl0p25.2016010100.f00.zarr", consolidated=True)
print(analysis_data)
