...
for module_name in modules_to_load:
    spec = importlib.util.spec_from_file_location(module_name, f'./graph_weather/models/data_assimilation/{module_name}.py')
    ...
    module.Data = Data
    module.HeteroData = getattr(__import__('torch_geometric.data', fromlist=['HeteroData']), 'HeteroData', None)
    module.torch = torch
    module.nn = torch.nn
...
spec = importlib.util.spec_from_file_location('interface', './graph_weather/models/data_assimilation/interface.py')
...
def test_integrate_da_with_model():
    ...
    integrated_model_pf = integrate_da_with_model(
        DummyModel(),
        da_strategy="particle",
        da_config={"num_particles": 6},
        ensemble_size=6
    )
    ...
    integrated_model_var = integrate_da_with_model(
        DummyModel(),
        da_strategy="variational",
        da_config={"iterations": 3},
        ensemble_size=4
    )
