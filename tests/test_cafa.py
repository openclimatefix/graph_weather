...
def test_encoder():
    ...
    ), f"Encoder output shape {output.shape} != expected {expected_shape}"
...
def test_processor():
    ...
    processor = Processor(dim=channels, depth=depth, num_heads=num_heads, mlp_ratio=mlp_ratio)
    ...
    assert (
        output.shape == expected_shape
    ), f"Processor output shape {output.shape} != expected {expected_shape}"
...
def test_decoder():
    ...
    assert (
        output.shape == expected_shape
    ), f"Decoder output shape {output.shape} != expected {expected_shape}"
...
def test_cafa_model():
    ...
    assert (
        output.shape == expected_shape
    ), f"CaFA output shape {output.shape} != expected {expected_shape}"
...
def test_cafa_full_pipeline():
    ...
    assert (
        output.shape == expected_shape
    ), f"Full pipeline output shape {output.shape} != expected {expected_shape}"
