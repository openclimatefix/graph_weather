...
def mock_model(config):
    ...
    def side_effect(x, masks=None, ensemble_size=None):
        ...
        for field in x:
            ...
            spatial_pattern = 0.1 * torch.sin(torch.linspace(0, 3.14, base.shape[2], device=base.device)).reshape(1, 1, -1, 1)
            ...
            if ensemble_size and ensemble_size > 1:
                ...
                variations = 0.02 * torch.randn(ensemble_size, *result.shape[1:], device=base.device)
...
class TestModelOperations:
    ...
    def test_model_loading_valid_path(self, mock_cuda, temp_model_file):
        ...
        with patch('graph_weather.models.atmorep.model.atmorep.AtmoRep', autospec=True) as mock_atmorep:
...
class TestDataHandling(DataHandlingTestBase):
    ...
    def test_dataset_getitem(self, config, dummy_data):
        ...
        with patch.object(ERA5Dataset, 'load_file') as mock_load, \
...
class TestTrainingComponents(TrainingTestBase):
    ...
    def test_loss_calculation_with_masks(self, config, dummy_data):
        ...
        small_data = {field: torch.randn(batch_size, time_steps, h, w) 
                      for field in config.input_fields}
        
        preds = {field: torch.randn(ensemble_size, batch_size, time_steps, h, w) 
                for field in config.input_fields}
    ...
    def test_training_initialization(self, mock_save, config, tmp_path):
        ...
        with patch('graph_weather.models.atmorep.model.atmorep.AtmoRep') as mock_model_class, \
    ...
    def test_training_with_resume(self, mock_save, config, tmp_path):
        ...
        with patch('graph_weather.models.atmorep.model.atmorep.AtmoRep') as mock_model_class, \
            ...
            try:
                train_atmorep(config, tmp_path / "era5_data", tmp_path / "output", 
    ...
    def test_checkpoint_saving(self, config, tmp_path):
        ...
        with patch('torch.save') as mock_torch_save:
            ...
            save_fn(mock_model, mock_optimizer, config, output_dir, 
                    epoch, best_val_loss, is_best, history)
...
class TestIntegration:
    ...
    def test_inference_with_normalization(self, mock_model, normalizer, config):
        ...
        small_data = {field: torch.randn(1, config.time_steps // 2, 
                                         config.spatial_dims[0] // 2, 
                                         config.spatial_dims[1] // 2) 
                     for field in config.input_fields}
        
        # Normalize inputs
        normalized_inputs = {field: normalizer.normalize(data, field) 
                            for field, data in small_data.items()}
        ...
        preds = {field: normalizer.denormalize(normalized_preds[field], field)
    ...
    def test_full_forecast_pipeline(self, mock_model, normalizer, config):
        ...
        small_data = {field: torch.randn(1, 3, 16, 32)  
                     for field in config.input_fields}
        ...
        forecast = {field: normalizer.denormalize(normalized_forecast[field], field)
    ...
    def test_model_training_epoch(self, tmp_path):
        ...
        try:
            ...
            model.forward = lambda x, masks=None, **kwargs: {field: x[field] * torch.nn.Parameter(torch.ones(1)) for field in x}
            ...
            batch = {
                field: torch.randn(1, small_config.time_steps, 
                                small_config.spatial_dims[0], 
                                small_config.spatial_dims[1], requires_grad=True) 
...
class TestModelArchitecture:
    ...
    def small_config(self):
        ...
        return AtmoRepConfig(
            ...
            batch_size=1,
            time_steps=2,
            input_fields=TEST_FIELDS[:2] 
        )
    
    @pytest.fixture(scope="class")
    def small_sample_data(self, small_config):
        """Fixture providing sample data for model testing."""
        return {field: torch.randn(1, small_config.time_steps, *small_config.spatial_dims) 
                for field in small_config.input_fields}
        ...
        for field in small_config.input_fields:
            ...
            if len(masked_output) > 0 and len(unmasked_output) > 0:
                ...
                assert not torch.allclose(
                    masked_output.mean(), unmasked_output.mean(), 
                    rtol=0.1, atol=0.1
                )
        ...
        for field in small_config.input_fields:
            ...
            for i in range(ensemble_size):
                for j in range(i+1, ensemble_size):
                    assert not torch.allclose(
                        outputs[field][i], outputs[field][j], 
                        rtol=1e-3, atol=1e-3
                    )
    ...
    def test_model_training_mode(self, small_config, small_sample_data):
        ...
        for field in small_config.input_fields:
            ...
            if not torch.allclose(train_outputs1[field], train_outputs2[field], rtol=1e-1, atol=1e-1):
                any_field_different = True
                break
        
        # Test passes if we found any difference
        assert any_field_different, "Training mode should produce different outputs with different random seeds"
    
    def test_autoregressive_property(self, small_config):
        """Test that model can use its own outputs as inputs for forecasting."""
        model = AtmoRep(small_config)
        
        initial_data = {field: torch.randn(1, 1, *small_config.spatial_dims) 
                      for field in small_config.input_fields}
        ...
        for step in range(forecast_steps):
            with torch.no_grad():
                ...
                for field in small_config.input_fields:
                    ...
                    forecasted_data[field] = torch.cat(
                        [forecasted_data[field], next_value], dim=1
                    )
...
class TestPerformanceAndScaling:
    ...
    def test_memory_usage(self, spatial_size):
        ...
        sample_data = {field: torch.randn(1, small_config.time_steps, 
                                          *small_config.spatial_dims, 
                                          device='cuda') 
                      for field in small_config.input_fields}
    ...
    def test_inference_speed(self, batch_size):
        ...
        config = AtmoRepConfig(
            ...
            batch_size=batch_size,
            time_steps=2,
            input_fields=TEST_FIELDS[:1]  
        )
        
        model = AtmoRep(config)
        
        small_data = {field: torch.randn(batch_size, config.time_steps, 
                                         *config.spatial_dims) 
                     for field in config.input_fields}
...
class TestRobustness:
    ...
    def test_zero_input(self, robust_config):
        ...
        zero_data = {field: torch.zeros(1, robust_config.time_steps, 
                                        *robust_config.spatial_dims) 
    ...
    def test_nan_handling(self, robust_config):
        ...
        data = {field: torch.randn(1, robust_config.time_steps, 
                                  *robust_config.spatial_dims) 
               for field in robust_config.input_fields}
    ...
    def test_single_precision(self, robust_config):
        ...
        fp32_data = {field: torch.randn(1, robust_config.time_steps, 
                                       *robust_config.spatial_dims, 
                                       dtype=torch.float32) 
                    for field in robust_config.input_fields}
    ...
    def test_device_transfer(self, robust_config):
        ...
        cpu_data = {field: torch.randn(1, robust_config.time_steps, 
                                      *robust_config.spatial_dims) 
                   for field in robust_config.input_fields}
