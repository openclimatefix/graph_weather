...
from graph_weather.data.icosahedral_graph_gen import (
    IcosahedralGrid,
...
def test_create_icosahedral_graph_function():
    ...
    assert (
        G1.number_of_nodes() > 12
    ), "create_icosahedral_graph(level=1) should have more than 12 nodes."
    assert (
        G1.number_of_edges() > 30
    ), "create_icosahedral_graph(level=1) should have more than 30 edges."
...
def test_vertices_on_unit_sphere():
    ...
    for i, v in enumerate(vertices):
        ...
        assert np.allclose(
            norm, 1.0, atol=1e-6
        ), f"Vertex {v} is not on the unit sphere (norm = {norm})."
