...
class TestWeatherStationReader:
    ...
    def test_get_observations_for_model(self, reader, sample_csv_file):
        ...
        time_filtered = reader.get_observations_for_model(time_range=(yesterday, tomorrow))
        assert time_filtered is not None

        # Test with variable filtering
        var_filtered = reader.get_observations_for_model(variables=["temperature", "pressure"])
    ...
    def test_synopticpy_integration(self, mock_init_synopticpy, reader):
        ...
        # Define a custom implementation that definitely calls get_observations
        def custom_fetch_from_synopticpy(self, start_date, end_date, stids, variables):
            ...
            response = self._synopticpy_client.get_observations(
                stids=stids, start=start_date, end=end_date, vars=variables
            )
        ...
        # Now call the method with the updated parameter name "variables"
    ...
    def test_interpolate_missing_data(self, reader):
        ...
        ds = xr.Dataset(
            data_vars={"temperature": (["time", "station"], temps.reshape(-1, 1))},
    ...
    def test_resample_observations(self, reader):
        ...
        ds = xr.Dataset(
            data_vars={"temperature": (["time", "station"], temps)},
            coords={"time": times, "station": stations},
        )
        ...
        assert resampled.temperature.values[0, 0] == 2.5
